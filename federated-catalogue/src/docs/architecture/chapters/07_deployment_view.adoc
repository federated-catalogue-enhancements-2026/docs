...
At startup the catalogue logs one INFO line per disabled family listing both activation paths.
Two ways to enable a family:

_Option A — Admin API (runtime, persisted):_

[source,http]
----
PATCH /admin/trust-frameworks/gaia-x
Content-Type: application/json

{"enabled": true}
----

_Option B — Environment variable (deployment-time, applied on every boot):_
