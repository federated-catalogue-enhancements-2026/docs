...
The four `schema.module.*.enabled` rows (admin-controlled booleans) gate the corresponding validator wherever it would otherwise run on the on-demand `POST /assets/validate` path. Schema validation is never run automatically on the credential upload path. See ADR 16 for the toggle semantics, including the asymmetric role of the OWL toggle (which gates base-class resolution).
