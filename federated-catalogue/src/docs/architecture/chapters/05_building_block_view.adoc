...
`AssetValidationService` provides on-demand validation of stored assets against registered validators, invoked via `POST /assets/validate` (CAT-FR-CO-05). It supports RDF assets (JSON-LD, Turtle, N-Triples, RDF/XML, Loire JWT), plain JSON assets, and XML assets.
...
On-demand validation against specific SHACL shapes is available via the `POST /assets/validate` endpoint (CAT-FR-CO-05).
