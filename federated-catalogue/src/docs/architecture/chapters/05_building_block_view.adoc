...
| Verification
a| Responsible for applying the defined verification rules on credentials (Verifiable Presentations or standalone Verifiable Credentials) and extracting claims. Also handles non-credential RDF assets (CAT-FR-GD-02). Accepts JWT-wrapped credentials per ICAM v24.07 (Gaia-X Loire) and generic VC 2.0 JSON-LD.
...
For RDF assets that are not Gaia-X Credentials (CAT-FR-GD-02), `CredentialVerificationStrategy` short-circuits the credential verification pipeline when `FormatDetector` returns `UNKNOWN` and delegates to `JenaAllTriplesExtractor` instead.
...
| Non-credential RDF | `UNKNOWN` | No recognized credential format — all triples extracted via `JenaAllTriplesExtractor` (CAT-FR-GD-02)
|===
