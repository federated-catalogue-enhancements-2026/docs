...
| Asset Link Service | Manages bidirectional links between machine-readable (RDF) assets and human-readable representations (e.g., PDF, DOCX). Link records are stored in the `asset_links` table; supplementary `fcmeta:hasHumanReadable` / `fcmeta:hasMachineReadable` RDF triples are written to the graph DB.
...
Links are created via `POST /assets/{mrId}/human-readable`. Content retrieval is exposed via
...
Each link is stored as two rows in the `asset_links` table — one per direction — using
`AssetLinkType` values `HAS_HUMAN_READABLE` (MR → HR) and `HAS_MACHINE_READABLE` (HR → MR).
The unique constraint `uq_asset_link` enforces a maximum of one HR per MR asset and one MR per
HR asset. Corresponding RDF triples (`fcmeta:hasHumanReadable`, `fcmeta:hasMachineReadable`) are
also written to the graph DB as supplementary metadata. Graph writes are non-fatal: a failure is
logged but does not roll back the PostgreSQL transaction.

**Cascade delete:** Deleting an MR asset also deletes its linked HR asset and all link rows.
Deleting an HR asset directly removes only the link rows; the MR asset is preserved.

**Graph rebuild:** `POST /admin/graph/rebuild` restores `fcmeta:hasHumanReadable` and
`fcmeta:hasMachineReadable` triples from the `asset_links` table.

The link metadata is stored using the following data model:
...
.Asset links table notes
****
. Two rows are inserted per logical link: one with `link_type = HAS_HUMAN_READABLE` (MR → HR) and one with `link_type = HAS_MACHINE_READABLE` (HR → MR).
. The unique constraint `uq_asset_link` covers `(source_id, target_id, link_type)`.
. `source_id` and `target_id` store asset IRIs (referencing `assets.subjectid`). No FK constraint is used — cascade delete is handled in application code by `AssetLinkService`.
. Auditing uses Spring Data JPA `@AuditingEntityListener` (not Hibernate Envers) — `created_by`, `modified_by`, `created_at`, `last_modified_at` are columns on the table itself, not in a separate `_aud` shadow table.
****
