...
Format classification is delegated to an **ordered chain of `FormatMatcher` beans** orchestrated by `CredentialFormatDetector`.
Each matcher is a Spring `@Component` carrying an `@Order(n)` annotation; the detector iterates the chain and the first matcher that returns a non-`UNKNOWN` classification wins.
Adding a new credential format is a matter of dropping in a new `FormatMatcher` bean — no edits to the strategy or detector itself.
...
classDiagram
    ...
    class VerificationStrategy {
        <<interface>>
        +verifyCredential(payload, verifySemantics, verifySchema, verifyVPSignatures, verifyVCSignatures) CredentialVerificationResult
    ...
    class CredentialFormatDetector {
        -matchers : List~FormatMatcher~
        +detect(headers, payload) CredentialFormat
    }
    class FormatMatcher {
        <<interface>>
        +match(context) CredentialFormat
    }
    class LoireMatcher {
        +match(context) CredentialFormat
    }
    class DanubeTechFormatMatcher {
        +match(context) CredentialFormat
    }
    ...
    CredentialFormatDetector --> FormatMatcher : iterates ordered chain
    FormatMatcher <|.. LoireMatcher
    FormatMatcher <|.. DanubeTechFormatMatcher
    LoireMatcher --> TrustFrameworkService : isEnabled("gaia-x")
    LoireMatcher --> TrustFrameworkRegistry : reads trust_anchor_url\nfrom bundle properties
...
The `resolveStrategy(ContentAccessor)` method always resolves to `CredentialVerificationStrategy`, which internally routes between credential verification and non-credential RDF extraction based on `CredentialFormatDetector` output.

`CredentialVerificationStrategy` implements all verification and claim extraction logic.
When the matcher chain returns a recognized credential format (`GAIAX_V2_LOIRE` or `VC2_DANUBETECH`), the full credential verification pipeline runs.
When the chain returns `UNKNOWN`, the strategy delegates to `JenaAllTriplesExtractor` to extract all triples from the raw RDF content — returning a `NonCredentialVerificationResult`.
...
`LoireMatcher` (`@Order(1)`) is the first matcher in the chain.
It only activates when the `gaia-x` trust-framework family is enabled in persistence — checked via `TrustFrameworkService.isEnabled("gaia-x")` — and reads the Trust Anchor Registry URL from the bundle's own `properties.trust_anchor_url`, rather than from application-level configuration.
`DanubeTechFormatMatcher` (`@Order(3)`) handles VC 2.0 JSON-LD credentials and runs after `LoireMatcher` so that a JWT carrying a Loire `typ` header is not absorbed by the generic VC 2.0 path.
...
| `properties:` block in `framework.yaml` | Framework-specific runtime values consumed by the bundle's own beans (e.g. `trust_anchor_url` read by `LoireMatcher`). Not interpreted by the registry.
|===
...
| `properties.trust_anchor_url` | URL of the Gaia-X Trust Anchor Registry consulted during Loire `x5u` trust-chain validation. Read at runtime by `LoireMatcher`.
|===
...
| VARCHAR(10) | content_kind | | Original upload modality: `RDF` or `NON_RDF`. Set at first upload; not changed by metadata enrichment.
|===
...
. The `content` column is nullable. For RDF assets, it stores the serialized RDF (e.g., JSON-LD). For non-RDF assets, content is stored only in the File Store and the column is initially `NULL`. When a non-RDF asset is enriched via metadata upload, the enrichment RDF is written into `content`; the asset's `content_kind` remains `NON_RDF`. The three asset metadata columns (`content_type`, `file_size`, `original_filename`) are backfilled with `content_type = 'application/ld+json'` for existing records.
...
flowchart TD
    ...
    Detect -->|Yes| ExtractIri[extractSubjectIdFromRdf]
    ExtractIri --> Lookup{AssetStore:\nfindEnrichableAsset?}
    Lookup -->|HR asset| Reject[422 Unprocessable Entity]
    Lookup -->|present\ncontentKind=NON_RDF| Enrich[enrichAsset:\nfilter + replace triples\n+ saveEnrichedContent]
    Lookup -->|absent| Format{FormatDetector:\ncredential format?}
    ...
    StoreCred --> Resp201[201 Created]
    StoreAsset --> Resp201
    Enrich --> Resp200[200 OK]
....
...
For RDF uploads whose primary subject IRI matches an existing non-RDF asset, the upload is treated as metadata enrichment:

* The payload must contain at least one triple whose subject is the asset's IRI (otherwise HTTP 400)
* If the matched asset has `asset_type = HUMAN_READABLE`, HTTP 422 is returned — HR assets cannot be enriched
* Otherwise, protected-namespace triples are filtered out, the asset's graph triples are replaced (`deleteClaims` → `addClaims`), and the raw RDF is persisted to `content`
* Enrichment requires an active graph backend; with `graphstore.impl=none` the request fails with HTTP 503
...
****
. `asset_type VARCHAR(50)` — nullable; values: `MACHINE_READABLE`, `HUMAN_READABLE`. `NULL` for standalone assets that are not part of an MR-HR pair.
...
**Metadata enrichment restriction:** Uploading an RDF graph via `POST /assets` with a subject IRI that resolves to a `HUMAN_READABLE` asset returns HTTP 422 Unprocessable Entity. HR assets are opaque documents intended for human consumption and cannot receive graph-queryable metadata.
...
| `LoireMatcher` | Format classification (per request) | Consults `TrustFrameworkService.isEnabled("gaia-x")` to decide whether to attempt Loire classification at all; reads `trust_anchor_url` from the bundle's `properties` for `x5u` trust-chain validation
