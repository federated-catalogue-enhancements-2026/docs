...
| `shapes.ttl` | SHACL shapes carried by the bundle. Currently NOT routed to the validation engine — the composite SHACL graph is still seeded from `defaultschema/shacl/` by `SchemaStoreImpl.initializeDefaultSchemas()`. Bundle-driven SHACL routing is a known follow-up (see ADR 10, Out of scope).
|===
