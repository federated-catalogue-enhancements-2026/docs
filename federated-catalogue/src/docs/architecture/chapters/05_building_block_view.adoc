...
The graph database holds the claims of verified, active credentials. Claims of credentials that fail verification are not added to the graph database. Claims of Deprecated, Expired or Revoked assets will be deleted from the Graph database.
...
Generically returning the credential files containing claims that influence query response is not possible. To get the relevant credential files, the query to the Graph Database can be formulated to return the Gaia-X entity that is the __credentialSubject__ of a Verifiable Credential. Then this can be used as a filter for the assets endpoint, to download the credential file.
...
| Ro-AS-A | Asset Administratorfootnote:[Renamed from `Ro-SD-A` (Self-Description Administrator) by CAT-NFR-01.] | ASSET_CREATE, ASSET_READ, ASSET_UPDATE, ASSET_DELETE, SCHEMA_READ
...
image:05_er_asset.png[ER Diagram representing an Asset]
////
This is the source code for the figure above. Directly added the figure due to a bug in mmdc.
NOTE: The table was renamed from `sdfiles` to `assets` and `sdhash` to `asset_hash` by CAT-NFR-01.
[mermaid, width=2000]
....
erDiagram
    ASSET {
