...
As specified by CAT-FR-SF-04, automatic SHACL schema validation during asset upload is disabled by default. This behavior is controlled by the `schema` verification property.
...
* SHACL Shapes remain uploadable and available for on-demand validation (CAT-FR-CO-05)
