...
After the strategy returns, the service consults the `verification.require-base-class` configuration flag (default `false`).
When the flag is `true` and the result is a credential whose `baseClass` is `null`, the credential type is unrecognised by every active trust-framework bundle and the request is rejected with a 400 listing the active bundles and their declared base classes.
When the flag is `false`, the credential is accepted with a `null` base class — the default trades the post-strategy safety net for the ability to ingest cross-framework payloads.
See <<_verification_toggle_interactions,Verification toggle interactions>> for how this flag composes with `verification.semantics`.
...
Three deploy-time flags shape what the credential pipeline rejects:
...
`verification.schema` (default `false`):: SHACL shape validation on extracted claims.
Composes with the SHACL admin module: when `schema=true` but the SHACL module is disabled at runtime, the request is rejected with `HTTP 400 module_disabled:SHACL`.

`verification.require-base-class` (default `false`):: Post-strategy null-`baseClass` safety net described above.
Returns `HTTP 400 "Credential type is not resolvable …"` when a credential survived the strategy with a `null` base class.

The unknown-`@type` case is the one where these flags interact.
The outcome depends on whether any trust framework is enabled and on the two relevant flags:

[options="header",cols="1,1,1,3"]
|===
| TF enabled? | `semantics` | `require-base-class` | Outcome on a credential whose `@type` does not match any registered base class
...
With both `semantics=true` and a trust framework enabled — the strict overlay — the 422 fires first and the post-strategy gate is effectively unreachable.
The default profile (`semantics=true`, `require-base-class=false`, no trust framework enabled out of the box) accepts cross-framework payloads with a `null` base class; the strict overlay flips `require-base-class` on and enables a trust framework, restoring the legacy reject-on-unknown-type behaviour.
...
Credentials whose type is only reachable through the subclass walk then fail base-class resolution and surface at the post-strategy `require-base-class` gate (see <<_verification_toggle_interactions,Verification toggle interactions>>): they are rejected with a 400 when the gate is on, accepted with a `null` base class when it is off (the default).
