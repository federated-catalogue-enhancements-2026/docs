...
sequenceDiagram
    ...
    Note over VerificationService: Schema validation disabled by default verifySchema false (CAT-FR-SF-04)
...
sequenceDiagram
    ...
    API -->> - User:OK
    loop For Each SD
        Revalidation ->> + SchemaValidationService:Validate SDs
        SchemaValidationService -->> - Revalidation:Result
        alt If NOK
...
The verification of SDs consists out of three parts (syntactic, cryptographic, semantic verification).
...
As specified by CAT-FR-SF-04, automatic schema validation against stored SHACL shapes has been **disabled in the upload verification flow**. Self-Descriptions are accepted regardless of their conformity to stored SHACL shapes. On-demand schema validation of stored assets against specific SHACL shapes is available through the dedicated `SchemaValidationService` (see <<_schema_validation_service>>) and will be exposed via a dedicated endpoint as part of CAT-FR-CO-05.
...
sequenceDiagram
    ...
    VerificationService --> VerificationService:[Verify Semantic]
    VerificationService ->> VerificationService:[Extract SD Type]
    Note over VerificationService: Schema validation disabled (CAT-FR-SF-04)
    VerificationService --> VerificationService:[Verify Signatures]
    VerificationService ->> VerificationService:[Extract Claims]
    VerificationService ->> VerificationService:[Extract Participant]
....
