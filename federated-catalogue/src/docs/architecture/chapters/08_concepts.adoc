...
The shipped Gaia-X compliance bundle (currently `gaia-x-2511`, release-tagged) wires the `GxdchComplianceClient` (client type `gxdch-loire`) to the canonical GXDCH HAProxy endpoint:
...
`https://compliance.gaia-x.eu/v2` is a HAProxy front-end that load-balances across eleven certified GXDCH operators.
...
The catalogue POSTs a Loire-conformant Verifiable Presentation JWT to the GXDCH compliance endpoint (e.g.
`/v2/api/credential-offers`).
...
.Sketch of the exchange (non-normative; see `GxdchComplianceClient` for the authoritative wire shape)
[source,http]
----
...
See ADR 16 for the storage rationale.
