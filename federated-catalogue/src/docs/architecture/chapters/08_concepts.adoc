...
* **Per-bundle role state.** One entry per (bundle, role) pair the operator has explicitly disabled; absence of an entry means the role is enabled by default.
...
* **Strict typed patch** — used for resources whose only mutable state is a boolean toggle (family enabled flag, role enabled flag, schema-module enabled flag).
