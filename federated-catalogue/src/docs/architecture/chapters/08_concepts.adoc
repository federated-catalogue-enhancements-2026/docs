...
Assets carry a `content_kind` field (values: `RDF`, `NON_RDF`) that records the original upload modality. The field is orthogonal to `AssetType` (`MACHINE_READABLE`, `HUMAN_READABLE`), which encodes the MR-HR linking status. Standalone assets (no MR-HR link) have `asset_type = NULL`.

[options="header",cols="1,1,2"]
|===
| content_kind | AssetType | Meaning
| `RDF` | _null_ | Standalone RDF asset (credential, ontology, shapes graph, …)
| `RDF` | `MACHINE_READABLE` | RDF asset linked to a human-readable counterpart
| `NON_RDF` | _null_ | Non-RDF binary/text asset (PDF, JSON, image, …); may be enriched
| `NON_RDF` | `MACHINE_READABLE` | Non-RDF asset linked to a human-readable counterpart; may be enriched
| `NON_RDF` | `HUMAN_READABLE` | Human-readable attachment; **cannot be enriched**
|===
...
. Persists the raw RDF payload to the `content` column (`AssetStore.saveEnrichedContent`), so the graph can be rebuilt from PostgreSQL at any time. The change is recorded in the Hibernate Envers audit trail with `changeComment = "Metadata enriched"`.
