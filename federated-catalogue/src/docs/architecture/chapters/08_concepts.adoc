...
Admin resource mutations follow a single convention: `PATCH /admin/<resource>/{id}` with an `application/merge-patch+json` body containing only the fields being modified.
The body schema enforces `minProperties: 1` (empty patches are rejected with `400`) and `additionalProperties: false` (unknown fields are rejected with `400`).
Absent fields are left unchanged; the API does not distinguish "absent" from "explicit null" at the wire level.

This convention applies uniformly to family-level trust-framework updates (`PATCH /admin/trust-frameworks/{id}`), per-bundle role state (`PATCH /admin/trust-frameworks/{bundleId}/roles/{roleName}`), and schema validation module toggles (`PATCH /admin/schema-validation/modules/{type}`).
